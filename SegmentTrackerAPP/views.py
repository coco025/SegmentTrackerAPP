...
# Establecer el punto de inicia de la aplicacion    
def index(request):
    return render(request, 'SegmentTracker_WEB/login.html', {})

# Función que carga las imágenes preprocesadas
@csrf_exempt
def load_images(dir):
    time.sleep(3) # Para evitar buscar archivos inexistentes
    onlyfiles = []
...
def load(request):   
    ...
    if("SegmentTrackerAPP" in path):
	    path = os.getcwd() + path.replace('SegmentTrackerAPP', "SegmentTrackerAPP/templates/SegmentTracker_WEB/")
    else:
	    path = os.getcwd() + "/SegmentTrackerAPP/templates/SegmentTracker_WEB/" + path
    
    data = open(path, "rb").read() 
    contentType = mimetypes.guess_type(path)
    return HttpResponse(data, content_type=contentType[0])

# Carga las imágenes del usuario para almacenarlas en el servidor
@csrf_exempt
...
def randomImages(request):
    ...
    randomFolder = os.getcwd().replace("\\", "/") + "/" + base_folder + "/DataSetRandom"
    #print("LOCATION: ", randomFolder)
    
    imagenes = os.listdir(randomFolder)

    random.shuffle(imagenes)

    #print(imagenes)
	
    cantImagenes = min(len(imagenes), random.randint(1, 7))
	
    for x in range(cantImagenes):
        filename = imagenes[x]
		
        src = os.path.join(randomFolder, filename)
...
def doSegmentation(request):
    ...
    if not os.path.exists(dirRes2):
        os.makedirs(dirRes2)
    unet_CellSegmentation.predict_web(dir + "/", dirRes, test_id)
...
def rastreo_iniciar(request):
    ...
    try:
        ...
        return HttpResponse("bad:Data Set inválido", content_type='text/plain')
    ...
    try:
        ...
        return HttpResponse("bad:El id: " + id_celula + " es inválido", content_type='text/plain')
